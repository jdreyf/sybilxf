...
```{r map}
ba_map <- map_seahorse(ba_sampled, model.nm="2.1A")
wa_map <- map_seahorse(wa_sampled, model.nm="2.1A")
```
...
stat.tab <- data.frame(t(apply(ad_flux_mat, MARGIN=1, FUN=function(x){
  ...
  p=suppressWarnings(wilcox.test(x[1:nsamples], x[(nsamples+1):length(x)])$p.value))
})))
